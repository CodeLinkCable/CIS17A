...
int mainGame(int totalCards, Deck fullDeck) 
{
    ...
    Bank bank(100000);
    PlayerBank pBank(startingMoney);
    ...
    while (leavingGame == false) {
        ...
        cout << "Your starting money is: " << pBank.getPMoney()<< " dollars." << endl;
        cout << "How much would you like to bet?  Input -1 to quit." << endl;
        cin >> betValue;
        if (betValue == -1) {
            break;
        }
        while (betValue <= 0 || betValue > pBank.getPMoney()) //Makes sure you can make the bet
        {
            cout << "How much would you like to bet?  Input -1 to quit." << endl;
            cin >> betValue;
        } 
        if(betValue>0 && betValue <= pBank.getPMoney())
        {
            ...
            if (currentTotal == 21 && dealerTotal != 21) //Player blackjacks
            {
                cout << "BLACKJACK!!!" << endl;
                enterLoop = false;
                pBank.addMoney(betValue * 2);
                cout << "Your money is now: " << pBank.getPMoney() << endl;
            }
                    ...
                    if (isSplit) //If the player is playing the split hand
                    {
                        ...
                        if (currentTotal > 21) //Checks for a bust in the split hand
                        {
                            ...
                            if (currentTotal > 21) //If the aces cant fix it, you bust
                            {
                                ...
                                pBank.loseMoney(betValue);
                                cout << "Lost the split hand...recovering original.  \nYour money is now " << pBank.getPMoney() << endl << endl << endl;
                                ;
                                splitHand.printHand();
            ...
            unusedCards.setCardsInHand(fullDeck.getCardsInHand());
            unusedCards.setSize(totalCards);
            if (pBank.getPMoney() <= 0) //Ends the game if the player has no money to spend.
            {
