...
//Template used to calculate which of your two
...
//Holds the players initials to be used
//During score tracking
struct playerScore 
{
    char firstI;
    char secondI;
...
//Prints the entire deck of cards (LEGACY)
void printDeck(Hand);
//Draws cards to be used
Hand drawCard(Hand, Hand, int);
//Resizes the hand after drawing a card
Hand resizeAfterDraw(Hand, int);
//Randomly generates a value to draw a card from
int randomizeCardIndex(int);
//Destroys card array
void destroy(Card*);
//Removes cards from the hand class
Hand removeCards(Hand);
//Changes string values to integers
int stringToInt(string);
//the main game of blackjack
int mainGame(int, Deck);
//Changes aces from 11 to 1 as needed
...
playerScore* readBin(fstream& in, int record) 
{ 
    //Reads and fills a struct with a file's contents
...
int mainGame(int totalCards, Deck fullDeck) 
{
    ...
    Bank bank(100000);
    PlayerBank pBank(startingMoney, "PLAYER");
    bool enterLoop = true; //!Check if the player enters the main gameplay loop IE wins or loses in the first two cards
    Hand playerHand("Player",0); //!Hand the player's cards are in
    Hand splitHand("Split",0); //!Hand the split cards are in
    Hand dealerHand("Dealer",0); //!Hand the dealers cards are in
    Hand unusedCards("Deck",0); //!The deck of the cards
    ...
    while (leavingGame == false) {
        ...
        cout << "Your starting money is: " << pBank.getSMoney()<< " dollars." << endl;
        cout << "How much would you like to bet?  Input -1 to quit." << endl;
        cin >> betValue;
        if (betValue == -1) {
            break;
        }
        while (betValue <= 0 || betValue > pBank.getSMoney()) //Makes sure you can make the bet
        {
            cout << "How much would you like to bet?  Input -1 to quit." << endl;
            cin >> betValue;
        } 
        if(betValue>0 && betValue <= pBank.getSMoney())
        {
            betsList.push_back(betValue);
            ...
            if (currentTotal == 21 && dealerTotal != 21) //Player blackjacks
            {
                cout << "BLACKJACK!!!" << endl;
                enterLoop = false;
                pBank.addMoney(betValue * 2,pBank);
                cout << "Your money is now: " << pBank.getSMoney() << endl;
            }
            if (dealerTotal == 21 && currentTotal != 21) //Dealer blackjacks
            {
                ...
                enterLoop = false;
                pBank.loseMoney(betValue * 2,pBank);
                cout << "Your money is now: " << pBank.getSMoney() << endl;
            }
            ...
            while (currentTotal <= 21 && enterLoop == true) 
            {
                    ...
                    if (isSplit) //If the player is playing the split hand
                    {
                        ...
                        if (currentTotal > 21) //Checks for a bust in the split hand
                        {
                            ...
                            if (currentTotal > 21) //If the aces cant fix it, you bust
                            {
                                ...
                                pBank.loseMoney(betValue,pBank);
                                cout << "Lost the split hand...recovering original.  \nYour money is now " << pBank.getSMoney() << endl << endl << endl;
                                splitHand.printHand();
                ...
                else if (choice == 'S') 
                {
                    ...
                    else if (currentTotal > dealerTotal) //Checks if the player beats the dealer
                    {
                        cout << "Win!" << endl;
                        pBank.addMoney(betValue, pBank);
                        cout << "Your money is now: " << pBank.getSMoney() << endl;
                    ...
                    else if (currentTotal < dealerTotal) //Checks if the player loses to the dealer
                    {
                        cout << "Lose!" << endl;
                        pBank.loseMoney(betValue,pBank);
                        cout << "Your money is now: " << pBank.getSMoney() << endl;
            ...
            unusedCards.setCardsInHand(fullDeck.getCardsInHand());
            unusedCards.setSize(totalCards);
            if (pBank.getSMoney() <= 0) //Ends the game if the player has no money to spend.
            {
    ...
    //Outputs the list of bets from the betsList vector once game is over
    cout << "Results for Bank: " << pBank.getBankName() << endl << endl;
    cout << "List of bets put in: " << endl;
    for(int i = 0; i < betsList.size(); i++)
    {
        cout << "Bet Number " << i+1 << ": " << betsList[i] << endl;
    }
    //Returns the player's bank value
    return pBank.getSMoney();
}